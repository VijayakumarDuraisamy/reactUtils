...
export var leadstageSchema = {
    stage_level: yup.number().typeError(NUM_TEXT).min(1, (MINMAX_TEXT)).max(6, (MINMAX_TEXT)).required(REQUIRED_TEXT),
    stage_name: yup.string().trim().max(50, (LIMIT_TEXT)).required(REQUIRED_TEXT).matches(/^[aA-zZ\s]+$/, (ALPHA_TEXT)),
    leadstage_description: yup.string().trim().nullable(true).max(250, (DESCRIPTION_TEXT)).matches(/^[aA-zZ\s]+$/, (ALPHA_TEXT)),
};
