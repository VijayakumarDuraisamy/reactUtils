...
export var customerActionSchema = {
    action_level: yup.number().typeError(NUM_TEXT).min(1, (MINMAX_TEXT)).max(5, (MINMAX_TEXT)).required(REQUIRED_TEXT),
    action_name: yup.string().trim().max(50, (LIMIT_TEXT)).required(REQUIRED_TEXT).matches(/^[aA-zZ\s]+$/, (ALPHA_TEXT)),
    action_description: yup.string().max(250, (DESCRIPTION_TEXT)).matches(/^[aA-zZ\s]+$/, (ALPHA_TEXT)),
};
