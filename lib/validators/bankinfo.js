...
export var bankSchema = {
    ...
    bname: yup.string().required(REQUIRED_TEXT).matches(/^[A-Za-z]+$/, "Enter Valid name"),
    //compcode: yup.string().required(REQUIRED_TEXT),
    //compname: yup.string().required(REQUIRED_TEXT),
    badd: yup.string().required(REQUIRED_TEXT).matches(/(?:[a-zA-Z]?[0-9])/, "Special charectors, Numbers not accepted"),
